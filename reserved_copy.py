...
def review_currency(currency_name):                                                    # відкуками конкретної валюти
    if request.method == 'DELETE':
        ...
        get_data(f"""INSERT INTO Review (id_currency, user_rating, average_rating, review_client, date_review) 
        VALUES ((SELECT id_currency FROM Currency 
                 WHERE name_currency='{currency_name.upper()}' ORDER by pricing_date DESC LIMIT 1), 
        '{request_data['user_rating']}', '{request_data['user_rating']}', '{request_data['review_client']}', 
        '{request_data['date_review']}')""")
